...
import puzzlePT from '../../data/puzzles/agent-parts_pt'
...
export default function AgentPartsPage() {
  ...
  if (phase === 'done') return <GameComplete slug="agent-parts" />

  if (phase === 'intro') return (
    <GameIntro
      title={isPT ? 'Partes do Agente' : 'Agent Parts'}
      type="group"
      description={
        isPT
          ? 'Agentes de IA são construídos a partir de componentes distintos. Agrupe estas 16 peças de arquitetura de agente em 4 categorias.'
          : 'AI agents are built from distinct components. Group these 16 pieces of agent architecture into 4 categories.'
      }
      howToPlay={isPT ? [

...
export default function SignalDropPage() {
  ...
  useEffect(() => {
    ...
    setIsSlow(track === 'elem-en' || track === 'elem-pt' || track === 'elem-es' || track === 'middle-pt')
  ...
  if (phase === 'intro') return (
    <GameIntro
      title={isES ? 'Caída de Señal' : isPT ? 'Queda de Sinal' : 'Signal Drop'}
      type="catch"
      description={
        isES
          ? 'Tú eres la paleta. La pelota lleva datos de entrenamiento. Mantén el peloteo para recolectar datos limpios — si la dejas caer, pierdes calidad.'
          : isPT
      ...
      howToPlay={isES ? [
        'Mueve la paleta para rebotar la pelota.',
        'Los combos aumentan tu puntaje — no dejes caer la pelota.',
        'Cada peloteo representa datos de entrenamiento siendo recolectados.',
        'Tu puntaje muestra cuántos datos limpios recolectaste.',
      ] : isPT ? [
